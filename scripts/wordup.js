...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord") === true) {
        ...
        // I moved this under #18 so the bad-attempt class
        // was not overridden by the tag-default class!!!

        // TODO 18 DONE
        // give the scoreChip appropriate css classes
        $(scoreChip).attr("class", "tag tag-default tag-sm");

        if (wordSubmission.isRealWord === false){
          $(scoreChip).removeClass("tag-default");
    ...
    // show word score chip as 'X' if it isn't a real word
    return wordChip;
}
